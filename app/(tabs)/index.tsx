...
export default function DashboardScreen() {
  ...
  useEffect(() => {
    const loadInitData = async () => {
      ...
      if (savedTrans) {
        setTransmission(savedTrans as "manual" | "matic");
      } else {
  ...
  const getRpmColor = (r: number) => {
    if (r > 5500) return "#ff4444";
    if (r > 4500) return "#e67e22";
    if (r > 3000) return "#f1c40f";
    return "#00ffcc";
  };
  ...
  const handleHudTap = () => {
    ...
    const newCount = hudTapCount + 1;
    setHudTapCount(newCount);

    if (newCount >= 2) {
      // Ketukan ke-2: Keluar HUD
      setHudTapCount(0);
      exitHudMode();
    } else {
      // Ketukan ke-1: Tunggu 800ms, kalau tidak ada ketukan ke-2, reset
        ...
        if (isNightTime) {
          setTimeout(() => {
            showAlert(
              ...
              "Hari sudah gelap. Ketuk ikon 'Mata' di atas untuk mengaktifkan HUD di kaca depan mobil.",
              "success",
            );
          }, 1500);
        }
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerRight}>
            {/* TOMBOL HUD */}
            <TouchableOpacity onPress={enterHudMode} style={styles.iconBtn}>
              <Ionicons
                name="eye-outline"
                size={18}
      ...
      <Modal visible={showTransModal} transparent animationType="fade">
        <View style={styles.modalBg}>
          <View style={[styles.modalBox, { alignItems: "center" }]}>
            ...
            <View style={{ flexDirection: "row", gap: 15, width: "100%" }}>
              <TouchableOpacity
                style={[styles.saveBtn, { flex: 1, backgroundColor: "#222" }]}
                onPress={() => saveTransmission("manual")}
              >
                <Text style={[styles.saveBtnText, { color: "#fff" }]}>
                  MANUAL
                </Text>
              ...
              <TouchableOpacity
                style={[
                  styles.saveBtn,
                  { flex: 1, backgroundColor: "#00ffcc" },
                ]}
                onPress={() => saveTransmission("matic")}
              >
      ...
      {/* ========================================================
    HUD (HEAD-UP DISPLAY) MODE — DESAIN BARU RAPIH & TEGAS
    ======================================================== */}
